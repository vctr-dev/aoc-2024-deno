...
export default async function (inputPath: string) {
	...
	const combinations = getComb(
		overridesFound.filter((v) => v.wrongBitsCount === 6),
	)
	...
	const newCombination = getComb(
		newOverridesFound.filter((v) => v.wrongBitsCount === 4),
	)
		.filter((v) => {
	...
	for (const { overrides } of newCombination) {
		...
		if (resultWrongBits === 0) {
			...
			console.log(overrides.flat().toSorted().join(','));
		}
	}
}
